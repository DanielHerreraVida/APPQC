...
class QCOrderSentViewModel : ViewModel() {
    ...
    val boxComposition: LiveData<List<Entities.BoxCompositionResponse>> get() = _boxComposition
    ...
    fun loadInspectionData(idBox: String) {
        viewModelScope.launch {
            try {
                ...
                boxInfoResult.getOrNull()?.let { _boxInfo.postValue(it) }
...
//                val result = service.sendQCHistorySent(
    ...
    fun updateBoxComposition(list: List<Entities.BoxCompositionResponse>) {
        _boxComposition.postValue(list)
    }
    ...
    suspend fun saveQCBox(
        ...
        return try {
            val result = service.saveQCHistorySent(
                idBox = idBox,
                ordNum = ordNum,
                awbNum = awbNum,
                telexNum = telexNum,
